dist_finder <- function(df, is_time_based, recov_th, recov_yrs,
  ...
  # Import Data
  cots_data_raw <- read.csv(paste0(
    data_path,
    "/GBRdata/CoTS_data.csv"
  ))
  cyc_data_raw <- read.csv(paste0(
    data_path,
    "/GBRdata/Cyclones_data.csv"
  ))
  dhw_data_raw <- read.csv(paste0(
    data_path,
  ...
  all_reefs_sf_cots <- st_intersects(
    distinct_sample_geom,
    cots_sf_unique
  )

  all_reefs_sf_cyc <- st_intersects(
    distinct_sample_geom,
    cyc_sf_unique
  )

  all_reefs_sf_dhw <- st_intersects(
    distinct_sample_geom,
    dhw_sf_unique
  )
  ...
  for (i in seq_len(nrow(df))) {
    ...
    dists <- cots_row %>%
      mutate(
        Hs4MW_value = cyc_row$Hs4MW_value,
        annMaxDHW_value = dhw_row$annMaxDHW_value
      )
    dists <- dists[dists$year < 2017, c(
      "REEF_ID", "geometry", "year",
