...
setwd("C:/Users/Pinkd/OneDrive - Queensland University of Technology/Documents/MPhil/Code") #PC
#setwd("~/MPhil/Code") #LAPTOP
...
## Import AIMS by-reef data ##
aims_reef <- read.csv("../Data/AIMS_manta-tow-by-reef/manta-tow-by-reef.csv")
...
## Summarise mmp data by reef
mmp_sum_by_reef <- mmp_data %>% group_by(MMP_SITE_NAME, DEPTH, REPORT_YEAR, GROUP_CODE) %>%
  summarise(COVER = mean(COVER), 
            LATITUDE = first(LATITUDE),
            LONGITUDE = first(LONGITUDE))

## Combine LTMP and MMP (select only Hard Cover from MMP)
hc_indices <- mmp_sum_by_reef$GROUP_CODE == "Hard Coral"
d5_indices <- mmp_sum_by_reef$DEPTH == 5
all_reefs <- data.frame(REEF_NAME = c(aims_reef$REEF_NAME, mmp_sum_by_reef$MMP_SITE_NAME[hc_indices & d5_indices]),
                        DEPTH = c(rep(NA, nrow(aims_reef)), mmp_sum_by_reef$DEPTH[hc_indices & d5_indices]),
                        LATITUDE = c(aims_reef$LATITUDE, mmp_sum_by_reef$LATITUDE[hc_indices & d5_indices]),
                        LONGITUDE = c(aims_reef$LONGITUDE, mmp_sum_by_reef$LONGITUDE[hc_indices & d5_indices]),
                        YEAR = c(aims_reef$REPORT_YEAR, mmp_sum_by_reef$REPORT_YEAR[hc_indices & d5_indices]),
                        COVER = c(aims_reef$MEAN_LIVE_CORAL, mmp_sum_by_reef$COVER[hc_indices & d5_indices]),
                        PROGRAM = c(rep("LTMP", nrow(aims_reef)), rep("MMP", sum(hc_indices & d5_indices))))
...
# all_reefs_tb <- as_tibble(all_reefs_sf)
# 
# # Perform a left join to merge geom_closest_cots and all_reefs_sf based on geometry
# merged_df <- left_join(all_reefs_tb, geom_closest_cots, by = "geometry")
# merged_df <- left_join(merged_df, geom_closest_cyc, by = "geometry")
...
for (i in seq_len(nrow(all_reefs_sf))) {
  ...
  if (length(nearest_index) == 0) {
    nearest_index <- unlist(geom_closest_cots$closest_index[geom_closest_cots$geometry == all_reefs_sf$geometry[i]])
  }
  ...
  if (length(nearest_index) == 0) {
    nearest_index <- unlist(geom_closest_cyc$closest_index[geom_closest_cyc$geometry == all_reefs_sf$geometry[i]])
  }
  ...
  if (length(nearest_index) == 0) {
    nearest_index <- unlist(geom_closest_dhw$closest_index[geom_closest_dhw$geometry == all_reefs_sf$geometry[i]])
  }
  ...
  if (i != 1 && all_reefs_sf$REEF_NAME[i-1] == all_reefs_sf$REEF_NAME[i]) { 
    # Get the year for the current row in all_reefs_sf
...
# Save to an rds file
saveRDS(all_reefs_sf, file = "../Data/all_reefs_sf.rds")
