...
p_calculator <- function(reef_data, mgmt_benefit) {
  ...
  for (reef in 1:nrow(reef_data)) {
    reef_data$r_single_mgd[reef] <- ifelse(is.na(as.numeric(reef_data$prob_s_recov[reef])),
      mean(as.numeric(reef_data$prob_s_recov), na.rm = TRUE) *
        (1 + mgmt_benefit),
      min(as.numeric(reef_data$prob_s_recov[reef]) *
        (1 + mgmt_benefit), 1)
    )
    reef_data$r_comp_mgd[reef] <- ifelse(is.na(as.numeric(reef_data$prob_c_recov[reef])),
      mean(as.numeric(reef_data$prob_c_recov), na.rm = TRUE) *
        (1 + mgmt_benefit),
      min(as.numeric(reef_data$prob_c_recov[reef]) *
        (1 + mgmt_benefit), 1)
    )

    # Calculate probability of recovered state for area if managed in single model
    # Prob disturbance at area
    di <- reef_data$prob_s_dist[reef] %>%
      as.numeric()
