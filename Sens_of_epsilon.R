...
epsilon_values <- seq(0.0125, 0.2, by = 0.0125)

# Initialize empty vectors to store results
num_single_disturbances <- numeric(length(epsilon_values))
num_compound_disturbances <- numeric(length(epsilon_values))
i = 1
# start forloop here:
...
results_df <- data.frame(epsilon = epsilon_values,
...
ggplot(data = dumbbell_df, 
       aes(x = epsilon, y = value, group = epsilon)) +
