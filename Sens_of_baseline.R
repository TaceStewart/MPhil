...
baseline_strs <- c("min", "mean", "max")

# Initialize empty vectors to store results
num_single_disturbances <- numeric(length(baseline_strs))
num_compound_disturbances <- numeric(length(baseline_strs))
i = 1
# start forloop here:
for (baseline_str in baseline_strs) {
  ...
  num_single_disturbances[i] <- nrow(single_dists)
  num_compound_disturbances[i] <- nrow(compound_dists)
  i <- i + 1
}
portrait_dims <- c(5,6)
if (inferBaseline) {
...
results_df <- data.frame(baseline = baseline_strs,
                         num_single = num_single_disturbances,
                         num_compound = num_compound_disturbances)
...
ggplot(data = dumbbell_df, 
       aes(x = baseline, y = value, group = baseline)) +
  ...
  scale_x_discrete(name = "Estimation of Baseline Value From Local Maxima",
                   labels = c("Maximum", "Mean", "Minimum")) +
  labs(y = "Number of Disturbances") + 
  theme(legend.position = c(.975, .975),
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "azure3"),
        legend.justification = c("right", "top"),
        legend.box.just = "left")
figname <- "/Sens_of_baseline"
